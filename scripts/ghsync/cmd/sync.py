...
def get_repositories(user: str, db_conn):
...
def get_issues(owner: str, repo: str, db_conn):
    """
...
@click.argument(
    "datatypes",
    nargs=-1,
    type=click.Choice(["issues", "prs", "repos"], case_sensitive=False),
)
...
@click.pass_context
def sync(ctx, datatypes, user, repo):
    """Syncs GitHub issues, pull requests, and repositories."""
    if not datatypes:
        print("Nothing to sync. Please specify issues, prs, or repos.")
        return
    ...
    if "issues" in datatypes or "prs" in datatypes:
        ...
        for repo_full_name in repos_to_sync:
            ...
            get_issues(owner, repo_name, db_conn)
